...
### 5.4 フェーズ×投資・人員・ユニットエコノミクス計画（基本シナリオ）

| 時点 | 人員 | 月次投資 | 累計投資 | 月商 | 粗利率（LLM原価後） | LTV12M/CAC | 単月損益 |
|---|---|---|---|---|---|---|---|
| 3ヶ月 | 3+開発兼務 | ¥600万 | ¥1,800万 | ¥1,000万 | 75% | 計測開始 | 赤字 |
| 6ヶ月 | 6 | ¥2,000万 | ¥0.8億 | ¥5,000万 | 75% | 2.0 | 赤字 |
| 12ヶ月 | 12 | ¥5,000万 | ¥3.5億 | ¥2.5億 | 78% | 3.0 | ほぼ均衡 |
| 24ヶ月 | 35 | ¥2億 | ¥15億 | ¥10億 | 80% | 5.0 | **黒字必須** |
| 36ヶ月 | 70 | ¥3.5億 | ¥45億 | ¥22.4億 | 80% | 8.0 | 黒字 |
| 60ヶ月 | 150 | ¥7億 | — | ¥50億 | 82% | 10 | 黒字（海外投資込み） |

- 投資には人件費・LLM原価・マーケ費・ツールを含む。マーケ費のうち有料広告は新規獲得の20%以下を生む範囲に制限（Spine §2）
- 粗利率75%未満が2ヶ月続いたらLLM原価対策（リスク#1）を最優先タスク化
- 人員計画は16_GOS §2.1（組織図）・§9.4（フェーズ先行採用）と連動。ゲート未達時は次フェーズ採用を凍結

### 5.5 フェーズ×実験カテゴリの対応

各フェーズの実験リソース配分は 15_Experiment_Backlog §5.8 に従う（P1=ACT/RET中心 → P2=REV/REF → P3=REF/ACQ → P4以降=全域機動）。ロードマップの大型施策（相性占い・Wrapped・海外β）は、必ず先行小実験（15_Experiment §6の形式）でGo/No-Go材料を作ってから四半期オフサイトで判定する（16_GOS §6.1）。
...
### 6.1 四半期別マイルストーン一覧（ゲートまでの中間チェックポイント）

ゲートは四半期に1回しか判定できないため、各四半期末に中間マイルストーンを置き、軌道逸脱を早期検知する。

| 四半期 | フェーズ | 中間マイルストーン（月末判定） | 未達時の即時アクション |
|---|---|---|---|
| Q1 | P1 | 計測基盤稼働（記録率100%）/ Aha率 ≥ 36% / D30 ≥ 8%（速報） | 実験配分をACTへ全振り |
| Q2 | P2 | 3プラン稼働 / 転換率 ≥ 3.5% / シェア完了率 ≥ 3% / LINE連携率 ≥ 35% | REV/REF実験の倍速化、価格再設計の準備着手 |
| Q3 | P3前半 | 相性占いβのD30・招待CVR速報 / SEO流入が全流入の30%超 | 相性占いの体験再設計 or 招待インセンティブ変更 |
| Q4 | P3後半 | K-factor ≥ 0.20 / Wrapped初回のシェア率実測 / 月商 ≥ ¥2億ペース | ゲートC延長判定の事前準備（コンテンツ主導モデル試算） |
| Q5〜Q6 | P4前半 | K ≥ 0.30 / オーガニック比率 ≥ 70% / 解約率 ≤ 6% | 継続系へ再配分、マス投資検討の凍結 |
| Q7〜Q8 | P4後半 | 月商 ¥8〜10億 / 営業黒字化 / 6チーム体制移行完了 | 黒字優先で成長投資減速（ゲートD選択肢①） |
| Q9〜Q12 | P5 | 想起調査トップ2 / CAC ≤ ¥1,000 / 海外β開始（Q10）とD30速報（Q12） | ブランド投資の効果検証見直し / β市場変更 |
| Q13〜Q20 | P6 | 海外本格展開の市場別LTV/CAC / プラットフォーム収益比率 5%→10%→15% | 市場縮小・国内深化パス（ゲートF代替）切替 |

### 6.2 ゲート判定の運用ルール

- 判定は四半期オフサイト（16_GOS §6.1）で、Dataが作成した判定パッケージ（各条件の実測値・信頼区間・データ品質注記）に基づいて行う。**「惜しいから通過」は禁止**（条件の一部未達は延長かピボットのみ）
- 延長は最大1四半期・1回限り。延長期間中は次フェーズ投資（採用・大型開発・広告増額）を凍結する
- ゲート通過時は、通過の根拠数値をDecision Log（16_GOS §2.5）に記録し、次フェーズの前提（このD30、この転換率が続く前提で投資する）として明文化する
...
**シナリオ判定ルール（どのシナリオを歩いているかの機械判定）**:

| 判定 | 条件（四半期末に判定） | 発動するアクション |
|---|---|---|
| 強気入り | MAUと月商がともに強気線の90%超 | 信頼性・CS・LLMコスト対策の投資を前倒し増強。採用を1四半期前倒し |
| 基本圏内 | 基本線の±20%以内 | 計画通り。ゲート判定を予定通り実施 |
| 保守警戒 | 基本線の80%未満が1四半期 | 原因分解（§9.1〜9.3の診断フローは14_KPI §9）と回復プランを翌四半期OKRに組込み |
| ピボット審査 | 保守線相当（基本の60%未満）が2四半期連続 | 該当ゲートの未達扱い→ピボット選択肢（§6）の審査を発動。採用・広告を凍結 |
...
- 上表の「発生シグナル」列の監視指標は、四半期オフサイト（16_GOS §6.1）で全10件を点検する。シグナル点灯中のリスクは翌四半期OKRのKRまたは専任タスクとして必ず引き取る
- リスク#5（依存批判）・#6（有害出力）・#7（漏洩）はブランド・信頼の不可逆リスクであり、**発生確率が低くても対応投資を削らない**（成長速度と交換しない）。他のリスクは期待損失（確率×影響額）で投資判断してよい
- 新規リスクの追加は誰でも起票可。四半期レビューで採否を決め、常に10件±3件に保つ（薄く広いリスク表は誰も見ない）

---

## 10. 海外展開の詳細方針（Phase 5-6の実行設計）

| 項目 | 方針 |
|---|---|
| 市場順序 | ①繁体字圏（台湾・香港: 占い文化の親和性最大、時差ゼロ運用可）→ ②東南アジア（タイ・ベトナム: モバイル課金成長市場）→ ③英語圏（市場最大だが競合最多のため最後） |
| 参入形態 | 翻訳ではなく文化ローカライズ: 現地の占術（紫微斗数・風水等）をコア体験に統合。ローカルの監修者と提携 |
| 検証設計 | 各市場ともβ→ゲート判定の2段階。β基準: 6ヶ月でD30 ≥ 国内比60%、CAC回収 ≤ 18ヶ月。判定までマーケ投資は月¥3,000万上限（Phase 5の隔離予算） |
| 価格 | 購買力平価でプラン価格を再設計（ARPPU目標は市場別に設定し、グローバルブレンドで¥3,000相当を維持） |
| 体制 | 国内OSの輸出: 16_GOSのリズム・実験規約・ガードレールをそのまま適用。現地チームは最小3人（国内Phase 1と同型）から始める |
| 撤退基準 | §5.3の通り: 2市場×18ヶ月でLTV/CAC < 2なら海外縮小、国内深化パス（ゲートF代替）へ |

## 11. プラットフォーム化の詳細方針（収益15%の中身）

| 収益源 | 内容 | 開始時期 | 60ヶ月時点の月商目安 |
|---|---|---|---|
| 鑑定書PDF・デジタル商品 | 高単価の記念鑑定書（誕生・結婚・転機）¥1,500〜¥5,000 | Phase 5 | ¥2.5億 |
| グッズ・物販 | 世界観グッズ（星図・カード・文具）、EC提携 | Phase 5後半 | ¥1.5億 |
| API・メディア提携 | 鑑定エンジンのAPI提供（メディア・アプリ組込み）。ブランド毀損審査を通過した提携先のみ | Phase 6 | ¥2億 |
| 法人提携 | ウェルネス・エンタメ領域とのコラボ。HR・与信・保険等の判定用途は**倫理審査により原則禁止**（占いを人の評価に使わせない） | Phase 6 | ¥1.5億 |

- 合計 ¥7.5億/月 = 月商50億の15%（Spine §4の収益分解と一致）
- プラットフォーム収益はWRS（コア体験）を毀損しないことを導入条件とする。物販・提携の導線がWRS/WAUを-1pt以上下げた場合は導線を降格する
